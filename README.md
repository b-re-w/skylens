...
> 두 뷰어는 **같은 하나의 포인트클라우드 소스**(결정적 시드로 양쪽이 동일하게 생성)를 서로 다른 방식으로 소비합니다.
...
└─ src/
   ...
   ├─ data/
   │  ├─ sceneData.ts    절차적 붕괴현장 포인트클라우드 (스플랫 placeholder)
   │  ├─ paths.ts        드론 3대 프리셋 경로
   │  └─ detections.ts   탐지 마커 (사람/위험)
   ├─ drones/
